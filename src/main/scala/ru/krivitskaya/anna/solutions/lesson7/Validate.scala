...
object Validate extends ZIOAppDefault {
  ...
  // Проверьте все правила, соберите ошибки и верните их список при неудаче
  def validateForm(form: RegistrationForm): IO[List[String], Unit] = {
    val checks: List[IO[String, Option[Unit]]] = List(
      checkUsername(form),
      checkEmail(form),
      checkPassword(form),
      checkAge(form)
    )
    ZIO.validate(checks)(identity).unit
  }
