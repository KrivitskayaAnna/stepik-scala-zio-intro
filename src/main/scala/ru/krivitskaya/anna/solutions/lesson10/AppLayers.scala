...
object DatabaseConnection {
  val live: URLayer[Config, DatabaseConnection] = ZLayer.fromFunction(DatabaseConnection.apply _)
}
...
object Cache {
  val live: URLayer[Config, Cache] = ZLayer.fromFunction(Cache.apply _)
}
...
object AppLayers extends ZIOAppDefault {
  ...
  val manualLayer: ULayer[UserService] =
    (dbConfigLayer >>> DatabaseConnection.live) ++ (cacheConfigLayer >>> Cache.live) >>> UserService.live

  // Реализуйте автоматическую сборку, насколько это возможно, через ZLayer.make
  val semiAutoLayer: ULayer[UserService] = ZLayer.make[UserService](
    dbConfigLayer >>> DatabaseConnection.live,
    cacheConfigLayer >>> Cache.live,
    UserService.live
  )
