...
object ComplicatedDeps extends ZIOAppDefault {
  val configLayer =
  ...
  val dbCompletedLayer = configLayer >>> databaseLayer

  val reposCompletedLayers = dbCompletedLayer >>> (firstRepoLayer ++ secondRepoLayer)

  val servicesCompletedLayers = reposCompletedLayers >>> (firstServiceLayer ++ secondServiceLayer)

  // Реализуйте ручную сборку через операторы >>> и ++
  val manualLayersComplicated: ULayer[App] = (configLayer ++ servicesCompletedLayers) >>> appLayer
