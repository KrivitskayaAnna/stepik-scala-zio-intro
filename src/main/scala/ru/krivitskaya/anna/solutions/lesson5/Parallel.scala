...
object Parallel extends ZIOAppDefault {
  ...
  def fetchSequential: UIO[String] = {
    for {
      resA <- fetchFromService("A")
      resB <- fetchFromService("B")
      resC <- fetchFromService("C")
    } yield s"$resA, $resB, $resC"
  }

  // Параллельно: получите данные от всех трёх сервисов одновременно
  def fetchParallel: UIO[String] =
    fetchFromService("A").zipPar(fetchFromService("B")).zipPar(fetchFromService("C")).map { case (resA, resB, resC) =>
      s"$resA, $resB, $resC"
    }
  ...
  override def run =
    for {
      ...
      _ <- fetchSequential.timed.flatMap { case (duration, result) =>
        Console.printLine(s"  $result (${duration.toMillis}ms)")
      }
      _ <- Console.printLine("Parallel:")
      _ <- fetchParallel.timed.flatMap { case (duration, result) =>
        Console.printLine(s"  $result (${duration.toMillis}ms)")
      }
    } yield ()
}